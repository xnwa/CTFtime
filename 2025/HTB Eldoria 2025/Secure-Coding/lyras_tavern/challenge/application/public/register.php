...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $confirm_password) {
        $error = 'The sacred words do not match';
    } else {
        try {
...
<head>
  ...
  <title>Arcane Chronicles - Register</title>
...
<body class="bg-magical">
  ...
  <main class="flex-grow container mx-auto px-4 py-8 relative z-10">
    <div class="max-w-md mx-auto">
      <form id="registerForm" method="POST" class="bg-parchment p-8 border-2 border-amber-700/50 rounded-lg shadow-lg">
        <h2 class="text-2xl font-medieval text-amber-500 mb-6 text-center">Arise, Tarnished</h2>
        <?php if($error): ?>
        ...
        <?php endif; ?>
        <div class="mb-6">
          <label for="username" class="block text-amber-500 font-medieval mb-2">Choose Thy Title:</label>
        ...
        <div class="mb-6">
          <label for="confirm_password" class="block text-amber-500 font-medieval mb-2">Confirm Sacred Password:</label>
          <input type="password" id="confirm_password" name="confirm_password" class="w-full p-3 border border-amber-700/50 rounded-lg bg-black/30 font-medieval text-amber-100" required />
        </div>
        ...
        <p class="text-center mt-4 font-medieval text-amber-100">
          Already a Tarnished?
          <a href="/public/login.php" class="text-amber-500 hover:text-amber-300 transition-colors">Touch Grace</a>
        </p>
      </form>
    </div>
  </main>
